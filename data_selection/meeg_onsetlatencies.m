...
%                groups (between)
% cfg.timewin  - latency range [in ms] to look for onset
% cfg.baseline - negative value for baseline in ms
% cfg.srate    - sampling rate
%
% Optional Input [default]:
%
% cfg.direction - direction 'pos' or 'neg', ['pos']
% cfg.method    - 'ind', 'jackknife', or 'jackind' ['ind' - individual 
%                 subjects] see comments for more info
% cfg.calc      - mean calculation type: 'none', 'rms', 'abs', ['none']
% cfg.timevar   - time range +/- from grandaverage onset in ms, around
...
% Output:
%
% latency      - latency of single subjects; each cell contains one
%                between subject condition
%                every row (lat) contains one within subj cond
% grandmean    - grand-average values 
%
% -------------------------------------------------------------------
...
if ~isfield(cfg, 'data'),   error('Error: cfg data needs to be defined!'), end
if ~isfield(cfg, 'calc'),   cfg.calc = 'none'; end
if ~isfield(cfg, 'method'), cfg.method = 'peak'; end
if ~isfield(cfg, 'direction'),    cfg.direction = 'pos'; end
if ~isfield(cfg, 'bscond'), 
  for iB = 1:numel(cfg.data), cfg.bscond{iB} = ['b' num2str(iB)]; 
  end, 
end
if ~isfield(cfg, 'wscond'), 
  for iW = 1:size(cfg.data{1},4), cfg.wscond{iW} = ['w' num2str(iW)]; 
  end, 
end
...
%% run jacknifing in all between subject conditions
for iB = 1:numel(cfg.data)

  
  % grandavg computation
  ...
  elseif strcmp(cfg.direction, 'pos')
