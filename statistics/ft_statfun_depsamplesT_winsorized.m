function [s, cfg] = ft_statfun_depsamplesT_winsorized(cfg, dat, design)
...
if strcmp(cfg.computestat,'yes')
  ...
  % calculate the dependent samples t-statistics based on winsorized means
  if strcmp(cfg.computestat, 'yes')
    avgdiff = winsorized_mean(diffmat,cfg.percentile, 2);
    vardiff = winsorized_std(diffmat,cfg.percentile, 0, 2).^2;
...
function wm = winsorized_mean(x, perc, dim)

% WINSORIZED_MEAN
%
% Use as
%    wm = winsorized_mean(x, perc, dim)
% where
%    x is the input data
%    perc is the percentage of values that is replaced (0.5 * perc at each tail)
%    only dim = 1 or dim = 2 supported
%
% See also MEAN, STD, WINSORIZED_STD

frac = perc/100;
[m, n] = size(x);
if dim==1
  % take this sample and sort
  y = sort(x,1);
  % replace the tails
  Nrepl = round(0.5*frac*m);
  y(1:Nrepl,:) = y(1+Nrepl,:); %left tail
  y(end-(Nrepl-1):end,:) = repmat(y(end-Nrepl,:), Nrepl,1); %right tail
  % compute the mean
  wm = mean(y,1);
elseif dim==2
  % take this sample and sort in each row
  y = sort(x, 2);
  % replace the tails
  Nrepl = round(0.5*frac*n);
  y(:,1:Nrepl) = y(:,1+Nrepl); %left tail
  y(:,end-(Nrepl-1):end) = repmat(y(:,end-Nrepl),1, Nrepl); %right tail
  % compute the mean
  wm = mean(y,2);
else
...
function ws = winsorized_std(x, perc, flag, dim)

% WINSORIZED_STD
%
% Use as
%    ws = winsorized_std(x, perc, flag, dim)
% where
%    x is the input data
%    perc is the percentage of values that is replaced (0.5 * perc at each tail)
%    only dim == 1 or dim == 2 supported
%    Pass in flag == 0 to use the default normalization by N-1, or 1 to use N (see STD)
%
% See also MEAN, STD, WINSORIZED_MEAN
